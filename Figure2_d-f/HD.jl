...
path = "Figure2_d-f"
scale_list = [2, 2.5, 3, 3.5, 4, 4.5]  # Scale
...
SS_proba_list_v1 = Array{Any, 2}(undef, 6, 3)  # Steady-state distribution
version = 1
tstep = 0.1; # Sampling time step
ssa_SS_P = readdlm("$path/data/$(VT)_cells_v$(version)/proba/cell_1.csv", ',')[:, end]  # SSA distribution for 10^7 trajectories
rho = [2.5 for _ in 1:VT]
d = [0.5 for _ in 1:VT]
u0 = zeros(N+1, VT)
u0[1, :] .= 1
graph = circle_graph(VT)  # Ring topology
for i in 1:6
    for j in 1:3
        p_path = p_path_list[i][j]
        @load "$path/model_params/$(p_path)" p  # Load training parameters
...
# ρ=1.0, d=1.0
table_list = [[0. for _ in 1:3] for _ in 1:6]
SS_proba_list_v2 = Array{Any, 2}(undef, 6, 3)  # Steady-state distribution
version = 3
tstep = 0.1; # Sampling time step
...
# savefig("Figure2_d-f/results/Figure2_d.pdf")

## plot Nueral-ODE and SSA
Figures = Any[]
version = 1 
...
# savefig("Figure2_d-f/results/Figure2_f_CD.pdf")

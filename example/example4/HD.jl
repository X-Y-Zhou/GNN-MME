...
for i in 1:6
    for j in 1:3
        ...
        @load "$path/model_params2/$(p_path)" p  # 加载训练参数
...
version = 3
tstep = 0.1; # 求解结果的采样时间步长
#ssa_SS_P = readdlm("data/$(VT)_cells_v$(version)/proba/cell_1.csv", ',')[:, end]  # 10^7 得到的 SSA 分布
ssa_SS_P = readdlm("$path/data/$(VT)_cells_v$(version)/proba/cell_1.csv", ',')[:, end]  # 10^7 得到的 SSA 分布
...
for i in 1:6
    for j in 1:3
        p_path = p_path_list[i][j]
        @load "$path/model_params2/$(p_path)" p  # 加载训练参数
        #@load "model_params/$(p_path)" p  # 加载训练参数
